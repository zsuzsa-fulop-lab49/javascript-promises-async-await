...
describe('Module 2', () => {
  ...
  it('should have an exported function named `fetchMovies` @declare-fetchmovies', () => {
    if (checkFileExists('index')) {
      ...
      expect(
        _.get(functionDeclaration, 'declaration.id.name', '') ||
          _.get(variableDeclaration, 'declaration.declarations[0].id.name', ''),
      ).to.equal(
        'fetchMovies',
  ...
  it('fetchWithTimeout should return a Promise that uses `setTimeout` @http-delay', () => {
    if (checkFileExists('services')) {
      ...
      if (variableDeclaration) {
        body =
          variableDeclaration.declaration.declarations[0].init.body.body[0];
        expect(
          _.get(variableDeclaration, 'declaration.declarations[0].id.name', ''),
        ).to.equal(
          'fetchWithTimeout',
        ...
        expect(
          _.get(body, 'argument.arguments[0].params[0].name', ''),
        ).to.equal(
          'resolve',
        ...
        expect(
          _.get(body, 'argument.arguments[0].body.callee.name', ''),
        ).to.equal(
          'setTimeout',
        ...
        body = functionDeclaration.declaration.body.body[0];
    ...
    if (checkFileExists('services')) {
      ...
      if (variableDeclaration) {
        const functionParams =
          variableDeclaration.declaration.declarations[0].init.params;
